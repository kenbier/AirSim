...
namespace AirSimUnity {
    ...
    public abstract class Vehicle : MonoBehaviour, IVehicleInterface {
        ...
        protected void Start() {
            ...
            airsimInterface.StartVehicleServer(AirSimSettings.GetSettings().LocalHostIP);
        }
        ...
        protected void LateUpdate() {
            ...
            if (isCapturingImages) {
                imageResponse = captureCameras[imageRequest.camera_id].GetImageBasedOnRequest(imageRequest);
                captureResetEvent.Set();                    //Release the GetSimImages thread with the image response.
        ...
        public ImageResponse GetSimImages(ImageRequest request) {
            imageResponse.reset();
            if (request.camera_id >= captureCameras.Count) {
        ...
        public CameraInfo GetCameraInfo(int cameraId) {
            ...
            foreach (DataCaptureScript capture in captureCameras) {
                if (capture.GetCameraID() == cameraId) {
        ...
        public bool SetCameraOrientation(int cameraId, AirSimQuaternion orientation) {
            foreach (DataCaptureScript capture in captureCameras) {
                if (capture.GetCameraID() == cameraId) {
        ...
        public bool PrintLogMessage(string message, string messageParams, int severity) {
            if (severity == 2) {
                Debug.LogError(message + " " + messageParams);
            } else if (severity == 1) {
                Debug.LogWarning(message + " " + messageParams);
            } else {
                Debug.Log(message + " " + messageParams);
            }
            return true;
        }
        ...
        private void SetUpCameras() {
            ...
            for (int i = 0; i < camerasParent.transform.childCount; i++) {
                ...
                camCapture.SetUpCamera(i, isDrone);
            }
        }
